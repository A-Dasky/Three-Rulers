...
def make_wall(val, x, y):
    ...
    walls = {'1': [WALL(x, y, 32, 4, params['bck_color']),    # Up
             ...
             '8': [WALL(x+28, y, 4, 32, params['bck_color'])  # Right
                  ],
             ...
             'T':[
                    WALL(x+28, y+22, 4, 16, params['bck_color']),  # Full, Half Length
                 ],
             'U':[
                    WALL(x, y+22, 4, 16, params['bck_color']),  # Full, Half Length
                 ],
             'V':[
                    WALL(x+28, y+6, 4, 16, params['bck_color']),  # Full, Half Length
                 ],
             'W':[
                    WALL(x, y+6, 4, 16, params['bck_color']),  # Full, Half Length
                 ],
             
             
             
             '0': [WALL(x, y, 32, 4, params['bck_color']),    # Up
                   WALL(x, y+28, 32, 4, params['bck_color']),  # Down
                   WALL(x, y, 4, 32, params['bck_color']),    # Left
                   WALL(x+28, y, 4, 32, params['bck_color'])  # Right
                  ],
             }
    return walls[val]
